# coding=utf-8
"""
Classical ViT training script - CIFAR-10 binary classification
...
from models.configs import get_cifar10_tiny_config
from utils.scheduler import WarmupLinearSchedule, WarmupCosineSchedule
from utils.early_stopping import EarlyStopping
from utils.dist_util import get_world_size
from data.Cifar10.dataloader import load_cifar10_binary
...
def setup(args):
    ...
    # Only load pretrained weights when pretrained_dir is specified and model_type is not tiny
...
def get_loader(args):
    """Get CIFAR-10 binary classification data loader"""
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()

    # CIFAR-10 data loader returns (trainset, testset) at once
    trainset, testset = load_cifar10_binary(
        train_transform=None,  # Use default transform
        test_transform=None,   # Use default transform
    ...
    # Distributed training: non-master process does not need testset
    if args.local_rank not in [-1, 0]:
        testset = None
...
def train_with_tracking(args, model, train_loader, test_loader, run_idx=None):
    ...
    val_accs = []
    train_accs = []
    val_losses = []  # [CIFAR10-specific] Validation loss record, only needed for CIFAR10 plotting
    steps = []
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.eval_every == 0 and args.local_rank in [-1, 0]:
            ...
            val_losses.append(val_loss)  # [CIFAR10-specific] Validation loss record
    ...
    return {
        ...
        'val_losses': val_losses,  # [CIFAR10-specific] Validation loss record
        'steps': steps
    }
...
def summarize_and_plot(results, model_name, args):
    ...
    if len(results) == 1:
        ...
        save_data = {
            ...
            # [CIFAR10-specific] Historical data records for plotting
    ...
    # [CIFAR10-specific] Collect historical data from all runs for plotting
    ...
    save_data = {
        ...
        'best_aucs': best_aucs,
        # [CIFAR10-specific] Historical data from all runs for plotting
...
def main():
    ...
    parser.add_argument("--model_type", choices=["ViT-B_16", "ViT-B_32", "ViT-L_16",
                                                 ...
                                                 "ViT-Tiny", "ViT-Tiny-1", "ViT-Classical-FashionMNIST",
                                                 "ViT-Classical-CIFAR10"],
                        default="ViT-Classical-CIFAR10",
                        help="Which variant to use. Note: ViT-Classical-CIFAR10 now uses the optimized Tiny config (4 layers, 256 hidden, 4x4 patches).")
    ...
    parser.add_argument("--img_size", default=32, type=int,
                        help="Resolution size (32 for CIFAR-10 native)")
