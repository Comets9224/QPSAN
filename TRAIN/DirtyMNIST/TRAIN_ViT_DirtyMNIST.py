...
from models.configs import get_classical_fashion_mnist_config
...
def setup(args):
    ...
    model = VisionTransformer(config, args.img_size, zero_head=True, num_classes=num_classes, in_channels=1)  # DirtyMNIST fixed to single channel
...
def get_loader(args):
    """Get DirtyMNIST binary classification data loader"""
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()

    # Compute fast_ratio from noise_ratio
    # If noise_ratio is specified, training set uses corresponding noise ratio, validation set forced clean
    if args.noise_ratio is not None:
        train_fast_ratio = 1.0 - args.noise_ratio
        test_fast_ratio = 1.0  # Validation set always clean
    ...
    # Load training set (stratified sampling for consistent clean/blurred ratio)
    trainset = load_dirty_mnist_binary(
        ...
        fast_ratio=train_fast_ratio,  # FastMNIST (clean samples) ratio
        img_size=args.img_size
    )

    # Load test set (Validation set always clean, decoupled from training noise)
    testset = load_dirty_mnist_binary(
        ...
        fast_ratio=test_fast_ratio,  # Validation set always clean (in noise_ratio mode)
        img_size=args.img_size
...
def main():
    ...
    parser.add_argument("--model_type", choices=["ViT-B_16", "ViT-B_32", "ViT-L_16",
                        ...
                        default="ViT-Classical-FashionMNIST",
    ...
    # DirtyMNIST fixed to single channel, in_channels parameter not needed
    ...
    # DirtyMNIST binary classification parameters
