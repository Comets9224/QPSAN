# coding=utf-8
"""
MLP585 ViT Training Script - FER2013 Binary Classification

Train 585-parameter MLP attention ViT using MLPVisionTransformer585 (classical ablation of quantum attention).
MLP585 attention: ~585-parameter 3-layer MLP scoring function (Linear(4,64)->Tanh->Linear(64,4)->Tanh->Linear(4,1)->Sigmoid), replacing quantum circuit (5 parameters).
FER2013 dataset: 48x48 grayscale, 7 expressions (0=Angry, 1=Disgust, 2=Fear, 3=Happy, 4=Sad, 5=Surprise, 6=Neutral)
"""
...
# Add project root to sys.path for subdirectory execution
...
from models.MLPVIT import MLPVisionTransformer585, MLP_CONFIGS
from utils.scheduler import WarmupLinearSchedule, WarmupCosineSchedule
from utils.early_stopping import EarlyStopping
from utils.dist_util import get_world_size
from data.FER2013.dataloader import load_fer2013_binary
    ...
    def compute_metrics(labels, preds, probs=None):
        ...
        if probs is not None:
            try:
                ...
                # If computation fails (e.g., only one class), set default
                metrics['auc'] = 0.0
...
class MetricsSummary:
    ...
    def add_run(self, best_metrics):
        """
        Add best metrics from one run

        Args:
            best_metrics: dict with accuracy, precision, recall, f1, auc, confusion_matrix
        """
    ...
    def compute_summary(self):
        ...
        # Save confusion matrix from best run
...
def save_model(args, model, run_idx=None):
    ...
    Args:
        ...
        model: Model
        run_idx: Run index (for multi-run)
    """
    model_to_save = model.module if hasattr(model, 'module') else model

    # Use path utils to get checkpoint path
...
class CheckpointManager:
    ...
    - save_all=True: Save checkpoint at every eval step, update best checkpoint
    """
    ...
    def __init__(self, args, run_idx=None):
        ...
        self.best_acc = 0.0  # Reset at each run start, track best within this run
        self.best_epoch = None  # Record best epoch within this run
        self.best_checkpoint_path = None  # Record current best checkpoint path
    ...
    def should_save(self, current_acc):
        ...
        if self.save_all:
            # Save all: save every time
            should_save = True
        else:
            # Save best only: save only when accuracy improves
            should_save = is_best
    ...
    def save(self, model, step, current_acc, epoch=None):
        ...
        if self.save_all:
            # Save checkpoint with step and epoch labels
            ...
            if is_best:
                # Use same naming format (suppress save log)
            ...
            # Save best checkpoint only, delete old best checkpoint
            if is_best:
                # Delete all old checkpoint files in directory
                import glob
                ...
                for old_file in old_checkpoints:
                    try:
                        ...
                        pass  # If delete fails, continue saving new one

                # Save new best checkpoint using {epoch+1:03d}epoch.bin format (3 digits, up to 999 epochs)
                best_filename = f"{epoch+1:03d}epoch.bin"
                best_path = os.path.join(self.checkpoint_dir, best_filename)
                torch.save(model_to_save.state_dict(), best_path)
                self.best_acc = current_acc
...
def setup(args):
    # Prepare model
    config = MLP_CONFIGS[args.model_type]

    # Binary classification task
    num_classes = 2

    model = MLPVisionTransformer585(config, args.img_size, zero_head=True, num_classes=num_classes, in_channels=args.in_channels)
    model.to(args.device)

    # Load pretrained weights only when pretrained_dir is specified and model_type is not tiny
...
def valid(args, model, writer, test_loader, global_step):
    ...
    Returns:
        dict: Metrics including accuracy, precision, recall, f1, auc, confusion_matrix
    """
...
def get_loader(args):
    """Get FER2013 binary classification data loader"""
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()

    # Load training set
    trainset = load_fer2013_binary(
...
def train_with_tracking(args, model, train_loader, test_loader, run_idx=None):
    ...
    """
    # ✅ Bug fix: move plotter init outside if block to avoid NameError in non-main processes
    plotter = None
    if args.local_rank in [-1, 0]:
        # Use path utils to get TensorBoard log path
    ...
    losses = AverageMeter()
    global_step, current_run_best_acc = 0, 0  # Fix: renamed to avoid conflict with checkpoint_manager.best_acc
    best_metrics = None  # Store best metrics dict
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # Update progress bar (show epoch progress only, no instantaneous loss)
        ...
        if (epoch + 1) % args.eval_every == 0 and args.local_rank in [-1, 0]:
            ...
            # Save via checkpoint_manager (includes epoch info)
            ...
            if accuracy > current_run_best_acc:  # Use own variable for comparison
                ...
                best_metrics = metrics.copy()  # Copy metrics dict
    ...
    return {
        ...
        'best_metrics': best_metrics,  # Add complete best metrics dict
...
def summarize_and_plot(results, model_name, args):
    ...
    if len(results) == 1:
        ...
        # Save results using path utils (consistent with multi-run)
        ...
        save_data = {
            ...
            # New: history data fields (single-run format)
            'train_losses': result.get('train_losses', []),
            'val_losses': [],  # Single run does not record val_losses
            'train_accs': result.get('train_accs', []),
            'val_accs': result.get('val_accs', []),
            'all_train_losses': [result.get('train_losses', [])],  # Multi-run format compatible
            'all_val_losses': [],
            'all_train_accs': [result.get('train_accs', [])],
            'all_val_accs': [result.get('val_accs', [])],
            'summary': {
    ...
    # Multiple runs - summarize with MetricsSummary
    print("\n" + "="*70)
    ...
    # ========== New: extract metric list from each run ==========
    ...
    # ========== New: collect history data ==========
    all_train_losses = [r.get('train_losses', []) for r in results]
    all_val_losses = []  # Currently not recording val_losses
...
def main():
    ...
    parser.add_argument("--model_type", choices=["ViT-MLP585-FER2013"],
                        default="ViT-MLP585-FER2013",
    ...
    parser.add_argument("--img_size", default=48, type=int,
                        help="Resolution size (48 for FER2013 native)")
    parser.add_argument("--in_channels", default=1, type=int,
                        choices=[1, 3],
                        help="Input image channels (1 for grayscale FER2013 native, 3 for RGB). Default: 1 (native grayscale)")
    ...
    parser.add_argument("--learning_rate", default=5e-3, type=float,
                        help="The initial learning rate (default: 5e-3 for FER2013)")
    ...
    parser.add_argument("--early_stopping_patience", default=20, type=int,
                        help="Early stopping patience (default: 10 epochs without improvement). "
                             "Set to 0 or negative to disable early stopping.")
    parser.add_argument("--max_grad_norm", default=1.0, type=float,
                        help="Max gradient norm.")

    # FER2013 dataset parameters
    parser.add_argument("--data_dir", type=str, default="./data/FER2013/data",
                        help="Path to FER2013 data directory (containing train/ and test/ subdirectories)")
    parser.add_argument("--class_a", type=int, default=3,
                        help="First emotion class index (0=Angry, 1=Disgust, 2=Fear, 3=Happy, 4=Sad, 5=Surprise, 6=Neutral, default: 3=Happy)")
    parser.add_argument("--class_b", type=int, default=5,
                        help="Second emotion class index (default: 5=Surprise)")
    parser.add_argument("--train_samples_per_class", type=int, default=500,
                        help="Number of training samples per class (default: 500)")
    parser.add_argument("--test_samples_per_class", type=int, default=200,
                        help="Number of test samples per class (default: 200)")
    parser.add_argument("--augmentation", choices=['none', 'weak'],
                        default='none',
                        help="Data augmentation strength: none/weak (default: none)")
    ...
    # Determine dataset name (include class info to avoid output overwriting across class pairs)
    ...
    # Add paths to args for downstream use
    args.paths = paths
    args.output_dir = paths["output_dir"]  # Keep compatibility
    ...
    # Setup logging (simplified format, suppress timestamp prefix)
